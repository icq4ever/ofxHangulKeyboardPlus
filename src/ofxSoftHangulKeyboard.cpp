...
ofxSoftHangulKeyboard::ofxSoftHangulKeyboard() {
	initializeHangulKeyMapTable();
	
	lastKeyComboCheckTimer = ofGetElapsedTimeMillis();
}

string ofxSoftHangulKeyboard::getBuffer(){
	return stringBuffer;
}

int ofxSoftHangulKeyboard::getLangState(){
...
//--------------------------------------------------------------
void ofxSoftHangulKeyboard::setup( ofBaseApp* _ofApp, int layout ) {
...
//--------------------------------------------------------------
void ofxSoftHangulKeyboard::setLayout(int layout) {
...
//--------------------------------------------------------------
void ofxSoftHangulKeyboard::reset() {
//	for(int i=0; i<keys.size(); i++) {
//		delete keys[i];
//	}
	keys.clear();
...
void ofxSoftHangulKeyboard::initializeHangulKeyMapTable(){
...
//--------------------------------------------------------------
//ofxSoftKey& ofxSoftHangulKeyboard::addKey(int c) {
//
...
// 라벨
ofxSoftKey& ofxSoftHangulKeyboard::addKey(int c, string label) {
...
void ofxSoftHangulKeyboard::keyInput(int key) {
	int code = 0;
	if(key <0)	return;
...
//--------------------------------------------------------------
void ofxSoftHangulKeyboard::newRow() {
	keys.back()->isLastInRow = true;
}

void ofxSoftHangulKeyboard::toggleInputLanguage(){
...
void ofxSoftHangulKeyboard::draw(ofVec2f _position){
	draw(_position.x, _position.y);
}

void ofxSoftHangulKeyboard::draw(float x, float y){
	setPosition(x, y);
	draw();
}

void ofxSoftHangulKeyboard::draw() {
...
int ofxSoftHangulKeyboard::getHangulMapCode(){
	return hangulMapCode;
}

void ofxSoftHangulKeyboard::setPosition(ofVec2f _pos){
	setPosition(_pos.x, _pos.y);
}

void ofxSoftHangulKeyboard::setPosition(float _x, float _y){
	position.x = _x;
	position.y = _y;
...
//--------------------------------------------------------------
void ofxSoftHangulKeyboard::setPadding(int top, int right, int bottom, int left) {
	for(auto &key: keys){
...
void ofxSoftHangulKeyboard::setTextColor(ofColor c){
	for(auto &key: keys){
		key->setTextColor( c );
	}
}
...
//--------------------------------------------------------------
void ofxSoftHangulKeyboard::setTextBGColor(ofColor c){
	for(auto &key: keys){
...
//--------------------------------------------------------------
void ofxSoftHangulKeyboard::setBorderColor(ofColor c){
	for(auto &key: keys){
...
//--------------------------------------------------------------
void ofxSoftHangulKeyboard::setHoverColor(ofColor c){
	for(auto &key: keys){
		key->setHoverColor( c );
	}
}
...
//--------------------------------------------------------------
void ofxSoftHangulKeyboard::setClickColor(ofColor c){
	for(auto &key: keys){
		key->setClickColor( c );
	}
}

void ofxSoftHangulKeyboard::setRoundness(float r){
	for(auto &key: keys){
		key->setRoundness(r);
	}
}
